...
<?php if($project_presentations->count() > 0) { ?>	
		<div class="row-fluid">
					<div class="span12">
						<div class="box">
							<div class="box-title">
							...
							<div class="box-content nopadding">
								<table class="table table-hover table-nomargin table-bordered">
									...
									<tbody>
									<?php foreach($project_presentations as $project_presentation) { ?>
											<tr>
												<td><?php echo $project_presentation->report_name; ?></td>
												<td><a onclick="return confirm('Are you sure?')" href="{{ Request::root() }}/admin/projects_type/delete/{{ $project_id }}/{{ $project_presentation->id }}/?type=project_presentations"><i class="icon-remove-sign"></i> Delete</a></td>	
											</tr>
									<?php } ?>
									</tbody>
								</table>
	...
	<?php } ?>
	<h4>Upload Project Presentations</h4>
	<div class="box-content nopadding">
		<div class="plupload_presentation">
		</div>
	</div>	
</div>
<script>
	if($('.plupload_presentation').length > 0){
		$(".plupload_presentation").each(function(){
			var $el = $(this);
			$el.pluploadQueue({
				runtimes : 'html5,gears,flash,silverlight,browserplus',
				url : '<?php echo Request::root();?>/admin/projects/upload_project_presentations/' + '<?php echo $project->id;?>',
				max_file_size : '40mb',
				chunk_size : '40mb',
				unique_names : true,
			...
			if($el.hasClass("pl-sidebar")){
				...
				upload.bind("UploadComplete", function(up){
					location.href = '<?php echo Request::root()?>/admin/projects/manage_project/' + <?php echo $project->id?> + '?tab=3';
				});		
		});
}
</script>